...
import { Wallet, CreditCard, Mail, User, Phone, LogOut, Key, Lock, X, CheckCircle2, AlertCircle, Eye, EyeOff, Pencil } from 'lucide-react';
...
export const ProfileTab: React.FC<Props> = ({ profile, onUpdateProfile, onSignOut }) => {
  ...
  const handleEditNameSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (currentUser) {
        ...
        await setDoc(userRef, { name: trimmed, fullName: trimmed }, { merge: true });
      }
      ...
      // Fallback update local state if Firestore write fails
      if (onUpdateProfile) {
        onUpdateProfile({ name: trimmed });
      }
        ...
        <div>
          <div className="flex items-center justify-center gap-2">
            ...
            <button
              ...
              id="edit-profile-name-pencil-btn"
              className="w-8 h-8 rounded-full bg-indigo-50 hover:bg-indigo-100 text-indigo-600 flex items-center justify-center transition-all cursor-pointer active:scale-90 border border-indigo-200"
              title="Edit Name"
            >
              <Pencil size={15} />
            </button>
            ...
            >
              ...
              <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h3 className="font-extrabold text-slate-900 text-base">Edit Name</h3>
                    <p className="text-[11px] font-medium text-slate-400">Update your account display name</p>
                  </div>
                </div>
                <button
              ...
              {/* Form */}
              <form onSubmit={handleEditNameSubmit} className="space-y-4 text-left">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full px-3.5 py-2.5 bg-slate-50 border-2 border-slate-200 rounded-xl text-xs font-bold text-slate-900 outline-none focus:border-indigo-600 focus:bg-white transition-all"
                    required
                    autoFocus
                  />
                </div>
